...
It talks to `services/api`. Design and test notes live in `lat.md/mobile/`;
...
| `make build ios` / `make build android` | Release build for one platform (`Runner.app`, or the app bundle). `make build` alone does both. |
...
| `make run` | Lists the devices Flutter sees and asks which to run on. Pass part of the name (`Air`) or the id. `make run ios` boots and shows the iPhone simulator; `make run android` does the same with an emulator. `DEVICE=<id>` skips the question. |
