...
- **Navigation**: `go_router` (flutter.dev) is the router, with the four destinations in a `StatefulShellRoute.indexedStack` and every screen addressable by path. Do not add `auto_route`, `go_router_builder`, `app_links` or the `animations` package, and do not write `RouterDelegate` code, without human approval. Leave `pageTransitionsTheme` unset and handle custom back through `PopScope` so predictive back keeps working. Details in `lat.md/mobile/mobile-architecture.md` under "Navigation".
