...
`BrandAppBar` (`lib/shell/brand_app_bar.dart`) is the one bar the four tabs share, so the bar never changes between tabs: the [[mobile-architecture#Brand lockup]] on the left and the Settings gear on the right, 64 high.
...
- **Website**: the script also writes `apps/site/public/logo{,-dark}.png`, the one-line logo with its tagline at twice its drawn 329 x 60, plus `favicon-32.png` and a flattened 180px `apple-touch-icon.png` ([[deployment#Site]]).
- **Notification icon**: Android draws a notification's small icon from its alpha alone, so `drawable-*dpi/ic_notification.png` is the glyph's white, trimmed and fitted into the 20dp live area of 24dp. The manifest names it as Firebase's `default_notification_icon`, with `default_notification_color` set to `@color/notification_colour`, the maroon of "reward" in the wordmark (`#933F53`).
...
The views are instances by urgency (opted-out credits left out, so no list shows them), the missed ledger, the totals (which do see opted-out credits, in their own figure), the use-soon, locked and captured lists, overlaps and the next reset.
...
- **The shell** is `StatefulShellRoute.indexedStack` whose builder renders `AppShell`: the brand bar over the content ([[mobile-architecture#Brand app bar]]), and the `NavigationBar` or `NavigationRail` for the width class ([[mobile-architecture#Responsive layout]]) around the content column, keeping each tab's scroll position across switches. The four `Destination`s are one list the bar and the rail both draw, so the order cannot differ.
...
- **Pictures** live in `lib/screens/welcome_heroes.dart` and draw the app's own widgets over the sample household (`sampleHousehold()` and `sampleToday` in `lib/logic/sample_household.dart`, which the Widget Previews also use), so a change to a row or a token shows on the slides too. Slide one, `UpcomingRewardsHero`, is Today's `TodayHeadline` (`lib/widgets/today_headline.dart`, drawn with a plain-text eyebrow rather than Today's `ScreenTitle`) over the first four `CreditRow`s: soon, then available, then locked. Slide two, `TimelyRemindersHero`, is a notification lookalike whose title and body are the first `notice` reminder `buildSchedule` makes for the sample household (default preferences with reminders on), above the `CreditRow` of that reminder's biggest credit, so the slide follows any change to the notification copy. Pinned by [[mobile-tests#Welcome heroes]].
...
It opens on the headline, counting only what is claimable, under its heading: the `ScreenTitle` reads "Today" and draws the date and eyebrow ("WED 16 SEP · UNCLAIMED, OPEN PERIODS"), since the brand and the gear sit in the shell's bar ([[mobile-architecture#Brand app bar]]). Below it come the use-soon rows with the reset countdown, up to three overlap cards on the section ground, the locked section with its own total, and the captured rows.
...
`SwipeRow` is the PWA's swipe-to-act ([[design#Screens]]) around `CreditRow`: swipe right to log the whole credit, left to silence it or opt out of it, parked open rather than fired, with a route for every gesture.

Material's `Dismissible` fires on release and cannot park, so the gesture is a `OneSequenceGestureRecognizer` of its own inside a `RawGestureDetector`, with the PWA's numbers: the gesture becomes a swipe only once horizontal movement passes 10 logical pixels, and bows out of the arena the moment vertical movement passes 10 and leads, so the list's own scroll is never stolen; each side opens 84 pixels per action, so the right side (one action) opens 84 and the left (two) 168, and the row rubber-bands past that width at 0.28; releasing past 55% of the side's width, or a flick over 0.45 pixels per millisecond in the row's direction, parks the row open at that width with the action buttons exposed side by side, and anything less springs back. Parking rather than firing is the point: an irreversible action should not be one accidental flick away, so the exposed "Log it", "Silence" or "Opt out" still needs a tap, which fires it and closes the row. A tap anywhere else closes an open row through `TapRegion.onTapOutside`. The mouse is ignored as in the PWA. The action sits behind the content in a `Stack` and is only built while the row is open, so a closed row keeps it out of the tree and the semantics; a `Transform.translate` slides the content.

`CreditRow` takes `onOpen`, `onLogAll`, `onToggleMute` and `onOptOut`; Today and Credits pass `onOptOut` only to a member who can write, since opting out changes the household's data. Tap opens the sheet through an `InkWell`; the bell on every row silences with the credit named in its label ("Silence reminders for Uber Cash"), because the row's title is not enough context in a long list; and each gesture is a `CustomSemanticsAction` on the row ("Log the full credit", "Silence" or "Unsilence", "Opt out"), so a screen reader and a switch reach both without the gesture, as the table in [[design#Accessibility#Pointer accelerators and their keyboard routes]] asks. The leading action exists only on a claimable row, and a captured or manual row has the gesture disabled; a locked row can still be silenced or opted out of. The callbacks are meant to be the shared `CreditActions` ([[mobile-architecture#Undo and the snackbar]]), so a swipe does exactly what the sheet does. Pinned by [[mobile-tests#Swipe row]]; the interactive rows have a Widget Preview.
