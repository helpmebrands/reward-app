...
- **Paths**: `/` Today, `/credits`, `/cards` and `/value` are the shell branches, the constants in `lib/shell/router.dart`. `/cards/new` (`Paths.newCard`), `/cards/:id` (`cardPath`) and `/benefit/:id` (`benefitPath`) are full-screen routes above the shell, the literal declared before the parameter so it wins; `/settings` follows with its screen, and `errorBuilder` the not-found screen. Value still renders `StubScreen`, a heading and one line, so the route is real before the screen is. `appRouter` takes an `initialLocation` so a test opens a screen directly.
