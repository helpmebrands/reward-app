...
void main() {
  ...
  group(
    'reminder sender against DATABASE_URL',
    () {
      late Connection db;
      late FakePush push;
      late TestApi api;
      ...
      Future<void> bobJoins() async {
        ...
        final r = await api
            .as('bob')
            .post('/v1/invites/$code/accept', {'confirmLeave': false});
        expect(r.status, 200, reason: '${r.body}');
      }
      ...
      test('a member who muted the card gets nothing; another member does', () async {
      ...
      test('the summary counts the caller\'s reminders and names the next', () async {
        await annsCard();
        await remindersOn('ann');
        await device('ann', 'phone', 'Europe/London');
