...
void main() {
  ...
  group('households against DATABASE_URL', () {
    late Connection db;
    late TestApi api;
    ...
    test('a code joins with its role once, and not after 7 days', () async {
      ...
      expect((await api.as('cat').post('/v1/invites/$old/accept')).status, 410);
      expect(
    ...
    test('an owner cannot leave while others remain', () async {
      ...
      final refused = await api.as('ann').post('/v1/invites/$elsewhere/accept');
      expect(refused.status, 409);
      expect(json(refused)['error'], 'owner has members');
    });

    // @lat: [[api-tests#Households#A removed member loses access at once]]
    test('a removed member is out on their next call, with nothing', () async {
      await api
          .as('bob')
          .post('/v1/invites/${await invite('ann', 'edit')}/accept');
      ...
      expect(members(await api.as('ann').get('/v1/household')), hasLength(1));
    });
