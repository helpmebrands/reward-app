...
void main() {
  ...
  group(
    'conversion against DATABASE_URL',
    () {
      late Connection db;
      late TestApi api;
      ...
      test(
        'the household’s captured totals and claims are unchanged',
        () async {
      ...
      test('the converted card copies the resolved terms and state', () async {
        ...
        final resolved = [
          for (final b
              in (json(before)['benefits'] as List)
      ...
      test(
        ...
        () async {
          ...
          expect(
            (await api.as('ann').post('/v1/cards/$id/convert')).status,
            404,
          );
        },
      );
