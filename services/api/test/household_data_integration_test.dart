...
void main() {
  ...
  group('household data against DATABASE_URL', () {
    late Connection db;
    late TestApi api;
    ...
    const platinum = {'templateId': 'amex-gold', 'anniversaryOn': '2024-05-01'};

    // @lat: [[api-tests#Household data#A template card's benefits are the resolved version]]
    test('a card from a template gets the resolved template credits', () async {
    ...
    test(
      ...
      () async {
        ...
        final edit = await api.as('ann').put('/v1/benefits/${benefit['id']}', {
          ...benefit,
          'valueCents': 1,
        });
    ...
    // @lat: [[api-tests#Household data#Opting out and reactivating are household state]]
    test('a credit opts out, converts, and reactivates from a date', () async {
      const at = '2026-05-01T09:00:00.000Z';
      final added = await addCard('ann', platinum);
      final cardId = (added['card'] as Map)['id'] as String;
      final id = list(added['benefits']).first['id'] as String;
      Future<Benefit> read(String id) async =>
          (await data('ann')).benefits.firstWhere((b) => b.id == id);

      final out = await api.as('ann').put('/v1/benefits/$id/state', {
        'optedOutAt': at,
      });
      expect(out.status, 200, reason: '${out.body}');
      expect(json(out)['optedOutAt'], at);
      expect((await read(id)).optedOutAt, at);
      expect((await read(id)).active, isTrue);

      // Converting the card keeps the opt-out on the new credit.
      final converted = await api
          .as('ann')
          .post('/v1/cards/$cardId/convert', {});
      expect(converted.status, 200, reason: '${converted.body}');
      final newId = (json(converted)['benefitIds'] as Map)[id] as String;
      expect((await read(newId)).optedOutAt, at);

      final back = await api.as('ann').put('/v1/benefits/$newId/state', {
        'optedOutAt': null,
        'trackedFrom': '2026-09-25',
      });
      expect(back.status, 200, reason: '${back.body}');
      final reactivated = await read(newId);
      expect(reactivated.optedOutAt, isNull);
      expect(reactivated.trackedFrom, '2026-09-25');

      for (final bad in [
        {'optedOutAt': 5},
        {'trackedFrom': 'soon'},
      ]) {
        final reply = await api.as('ann').put('/v1/benefits/$newId/state', bad);
